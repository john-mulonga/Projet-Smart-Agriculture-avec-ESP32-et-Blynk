...
#define RAIN_PIN 12  // Capteur de pluie (digital, 3 broches)
#define SOIL_PIN 27  // Capteur humidité sol (analogique AO)
#define RELAY_PIN 35 // Relais sur GPIO35

// --- Setup Blynk ---
BlynkTimer timer;

// Variable pour mode manuel/auto
...
void sendSensor() {
  ...
  if (!manualMode) {
    ...
    if (soilState == 1) {
      ...
      Blynk.virtualWrite(V4, 1); // Relais ON
      Serial.println("Mode AUTO → Sol sec → Pompe activée !");
    } else {
      digitalWrite(RELAY_PIN, LOW);
      Blynk.virtualWrite(V4, 0); // Relais OFF
